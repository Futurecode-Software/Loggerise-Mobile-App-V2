/**
 * Edit Promissory Note Screen
 *
 * Edit existing promissory note (senet).
 * Matches backend MobileUpdatePromissoryNoteRequest validation.
 */
...
export default function EditPromissoryNoteScreen() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Populate form with promissory note data
        setFormData({
        ...
        showError('Hata', 'Senet bilgileri yüklenemedi.');
        router.back();
      } finally {
        setIsLoading(false);
      }
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      success('Başarılı', 'Senet başarıyla güncellendi.');
      router.back();
    } catch (error: any) {
  ...
  if (isLoading) {
    return (
      <View style={styles.container}>
        <FullScreenHeader title="Senet Düzenle" showBackButton />
  ...
  return (
    <View style={styles.container}>
      <FullScreenHeader
        title="Senet Düzenle"
        subtitle="Senet bilgilerini güncelleyin"
        rightIcons={
          <TouchableOpacity
