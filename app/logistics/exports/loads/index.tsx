...
export default function ExportLoadsScreen() {
  ...
  // Refresh on screen focus - only if coming back from detail/edit
  useFocusEffect(
    useCallback(() => {
  ...
  const renderLoad = (item: Load) => {
    ...
    return (
      <StandardListItem
        ...
        onPress={() => router.push(`/logistics/load/${item.id}` as any)}
      />
    );
  };
      ...
      <FullScreenHeader
        ...
        rightIcons={
          <View style={{ flexDirection: 'row', gap: Spacing.md }}>
            <TouchableOpacity
              onPress={() => router.push('/logistics/load/new?direction=export')}
              activeOpacity={0.7}
            >
