/**
 * Beklenen Mal Detay Sayfası
 *
 * CLAUDE.md ilkelerine uygun
 * SectionHeader + InfoRow pattern'i
 * Varış Onayla / Kabul Onayla aksiyonları
 * useFocusEffect ile yenileme
 * ConfirmDialog ile silme onayı yok (bu sayfada silme yok)
 */
...
export default function ExpectedItemDetailScreen() {
  ...
  const handleConfirmArrival = async () => {
    ...
    try {
      ...
      Toast.show({
        type: 'error',
        text1: err instanceof Error ? err.message : 'Varış onaylanamadı',
        position: 'top',
        visibilityTime: 1500,
      })
    } finally {
  ...
  const handleConfirmReceiving = async () => {
    ...
    try {
      ...
      Toast.show({
        type: 'error',
        text1: err instanceof Error ? err.message : 'Kabul onaylanamadı',
        position: 'top',
        visibilityTime: 1500,
      })
    } finally {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.headerContainer}>
        ...
        <View style={[styles.headerContent, { paddingTop: insets.top + 16 }]}>
          ...
          <View style={styles.headerBar}>
            ...
            {/* Boşluk (düzenleme/silme yok bu sayfada) */}
      ...
      >
        ...
        {!isLoading && item && (
          <>
            {/* Durum Aksiyonları */}
            {(item.status === 'expected' || item.status === 'pending_receiving') && (
              <View style={styles.actionCard}>
                {item.status === 'expected' && (
                  <TouchableOpacity
                    style={[styles.actionButton, styles.actionButtonArrival]}
                ...
                {item.status === 'pending_receiving' && (
                  <TouchableOpacity
                    style={[styles.actionButton, styles.actionButtonReceiving]}
            ...
            {/* Tarih Bilgileri */}
            <View style={styles.card}>
              <SectionHeader title="Tarih Bilgileri" icon="calendar-outline" />
              <View style={styles.cardContent}>
                {item.expected_arrival_date && (
                  <InfoRow
                    label="Beklenen Varış"
                    value={formatDate(item.expected_arrival_date)}
                    icon="time-outline"
                    highlight
                  />
                )}
                {item.actual_arrival_date && (
                  <InfoRow
                    label="Gerçekleşen Varış"
                    value={formatDate(item.actual_arrival_date)}
                ...
                {item.received_date && (
                  <InfoRow
                    label="Kabul Tarihi"
                    value={formatDate(item.received_date)}
                    icon="checkmark-done-outline"
                  />
                )}
                {item.expected_loading_date && (
                  <InfoRow
                    label="Beklenen Yükleme"
                    value={formatDate(item.expected_loading_date)}
                    icon="car-outline"
                  />
                )}
              </View>
