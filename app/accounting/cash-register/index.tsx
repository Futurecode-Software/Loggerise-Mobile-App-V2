      ...
      <View style={styles.cardInfo}>
        ...
        {item.responsible_user && (
          <View style={styles.infoRow}>
            ...
            <Text style={styles.infoText} numberOfLines={1}>{item.responsible_user.name}</Text>
          </View>
        )}
      </View>
  ...
  return (
    <View style={styles.emptyState}>
      <View style={styles.emptyIcon}>
        <Ionicons name="wallet-outline" size={64} color={DashboardColors.textMuted} />
      </View>
      <Text style={styles.emptyTitle}>Henüz kasa yok</Text>
      <Text style={styles.emptyText}>
        Yeni kasa eklemek için sağ üstteki + butonuna tıklayın.
      </Text>
    </View>
  )
}
...
export default function CashRegistersScreen() {
  ...
  const getTotals = () => {
    ...
    cashRegisters.forEach((cr) => {
      if (!totals[cr.currency_type]) {
        totals[cr.currency_type] = { total: 0, count: 0, positive: 0, negative: 0 }
      }
      totals[cr.currency_type].total += cr.balance
      totals[cr.currency_type].count++
      if (cr.balance > 0) totals[cr.currency_type].positive++
      if (cr.balance < 0) totals[cr.currency_type].negative++
    })
    return totals
  }
  ...
  const handleCardPress = (item: CashRegister) => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light)
    router.push(`/accounting/cash-register/${item.id}`)
  }

  const handleNewPress = () => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium)
    router.push('/accounting/cash-register/new')
  }
