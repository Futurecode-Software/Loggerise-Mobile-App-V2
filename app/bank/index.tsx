...
import { Filter, Plus, Landmark, Copy, Check, Layers, DollarSign, Euro, PoundSterling, TrendingUp, ChevronLeft, ChevronRight, Building2 } from 'lucide-react-native';
...
export default function BankAccountsScreen() {
  ...
  const cardWidth = screenWidth - Spacing.lg * 2 - 40;

  const renderCarouselCard = ({ item, index }: { item: [string, number]; index: number }) => {
    const [currency, total] = item;
    const currencyBanks = banks.filter(b => b.currency_type === currency);
    const positiveCount = currencyBanks.filter(b => b.balance > 0).length;
    const negativeCount = currencyBanks.filter(b => b.balance < 0).length;
      ...
      <View style={[styles.carouselCard, { width: cardWidth }]}>
        ...
        <View style={styles.carouselHeader}>
          <View style={styles.carouselHeaderLeft}>
            ...
            <Text style={styles.carouselTitle}>{currency} Hesapları</Text>
          </View>
          <View style={styles.carouselBadge}>
            <Text style={styles.carouselBadgeText}>{currencyBanks.length} Hesap</Text>
          </View>
        </View>
