...
export default function TractorTrailerAssignmentsScreen() {
  ...
  const handleToggle = async (assignment: TractorTrailerAssignment) => {
    try {
      await toggleTractorTrailerAssignment(assignment.id);
  ...
  const handleDelete = (assignment: TractorTrailerAssignment) => {
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteTractorTrailerAssignment(itemToDelete.id);
  ...
  const renderAssignment = (item: TractorTrailerAssignment) => {
    ...
    const additionalInfo = (
      <View style={styles.additionalInfo}>
        ...
        <View style={styles.infoSection}>
          <Text style={[styles.infoLabel, { color: colors.textSecondary }]}>Römork:</Text>
          <Text style={[styles.infoValue, { color: colors.text }]}>{trailerInfo}</Text>
        </View>
        {item.notes && (
          <View style={styles.infoSection}>
    ...
    return (
      <StandardListItem
        ...
        title={`${item.tractor?.plate || 'Çekici'} - ${item.trailer?.plate || 'Römork'}`}
        ...
        actions={
          <View style={styles.actions}>
            ...
            <TouchableOpacity
              onPress={() => router.push(`/fleet/tractor-trailer/${item.id}` as any)}
              style={styles.actionButton}
              activeOpacity={0.7}
            >
    ...
    <View style={styles.container}>
      ...
      <View style={styles.contentCard}>
        ...
        search={{
          ...
          placeholder: 'Çekici veya römork plakası ile ara...',
        }}
        emptyState={{
          icon: Link2,
      ...
      <ConfirmDialog
        ...
        message={
          itemToDelete
            ? `${itemToDelete.tractor?.plate} - ${itemToDelete.trailer?.plate} eşleştirmesini silmek istediğinize emin misiniz?`
            : ''
        }
        confirmText="Sil"
        cancelText="İptal"
