/**
 * Driver-Tractor Assignment Detail Screen
 *
 * Sürücü-çekici eşleştirmesi detay ekranı.
 * Backend: GET /api/v1/mobile/filo-yonetimi/surucu-cekici-eslestirme/{assignment}
 */
...
export default function DriverTractorDetailScreen() {
  ...
  // Veri çekme
  const fetchAssignment = useCallback(async (showLoading = true) => {
    if (!assignmentId) {
    ...
    try {
      ...
      // Backend'de tek kayıt getiren endpoint yok, liste endpoint'i kullanıyoruz
      const response = await getDriverTractorAssignments({ page: 1, per_page: 100 })
      const foundAssignment = response.assignments.find(a => a.id === assignmentId)
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      await deleteDriverTractorAssignment(assignmentId)
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.headerContainer}>
        ...
        <View style={[styles.headerContent, { paddingTop: insets.top + 16 }]}>
            ...
            {isLoading ? (
              ...
              <View style={styles.headerTitleSection}>
                <Text style={styles.headerName} numberOfLines={1}>
                  {assignment.employee?.full_name || 'Sürücü'}
                </Text>
              </View>
            ) : (
              <View style={styles.headerTitleSection} />
          ...
          {isLoading ? (
            ...
            <View style={styles.summaryRow}>
              <View style={styles.summaryInfo}>
                <Text style={styles.summaryLabel}>Çekici</Text>
                <Text style={styles.summaryValue}>
                  {assignment.tractor?.plate || '-'}
                </Text>
              </View>
              <View
                style={[
          ...
          <>
            ...
            <View style={styles.card}>
              ...
              <View style={styles.cardContent}>
                <InfoRow
                  label="Sürücü Adı"
                  value={assignment.employee?.full_name || '-'}
                  icon="person-outline"
                  highlight
                />
                {assignment.employee?.phone_1 && (
                  <InfoRow
                    label="Telefon"
                    value={assignment.employee.phone_1}
                    icon="call-outline"
                  />
                )}
                {assignment.employee?.email && (
                  <InfoRow
                    label="E-posta"
                    value={assignment.employee.email}
                    icon="mail-outline"
                  />
                )}
              </View>
            ...
            <View style={styles.card}>
              ...
              <View style={styles.cardContent}>
                ...
                {assignment.tractor?.brand && (
                  <InfoRow
                    label="Marka"
                    value={assignment.tractor.brand}
                ...
                {assignment.tractor?.model && (
                  <InfoRow
                    label="Model"
                    value={assignment.tractor.model}
            ...
            <View style={styles.card}>
              <SectionHeader title="Eşleştirme Bilgileri" icon="link-outline" />
              <View style={styles.cardContent}>
                <InfoRow
                  label="Atanma Tarihi"
                ...
                <InfoRow
                  ...
                  icon={assignment.is_active ? 'checkmark-circle-outline' : 'close-circle-outline'}
                />
              </View>
            </View>
      ...
      <ConfirmDialog
        ...
        message={
          assignment
            ? `${assignment.employee?.full_name || 'Sürücü'} - ${assignment.tractor?.plate || 'Çekici'} eşleştirmesini silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.`
            : 'Bu eşleştirmeyi silmek istediğinizden emin misiniz?'
        }
        type="danger"
        confirmText="Sil"
        cancelText="İptal"
